...
def main():
    ...
    train_pairs = create_transformed_pairs_dataset(X_train, y_train, config_set2.BATCHSIZE, config_set2.N_FEATURES)
    train_batches = int(len(X_train) // config_set2.BATCHSIZE)

    val_pairs = create_transformed_pairs_dataset(X_val, y_val, config_set2.BATCHSIZE, config_set2.N_FEATURES)
    val_batches = int(len(X_val) / config_set2.BATCHSIZE)

    test_pairs = create_transformed_pairs_dataset(X_test, y_test, config_set2.BATCHSIZE, config_set2.N_FEATURES)
    test_batches = int(len(X_test) // config_set2.BATCHSIZE)

    # Create the model
    simclr_model = create_simclr_model(
        n_features=config_set2.N_FEATURES,
        f_r_units=config_set2.GNN_BASELINE_F_R_LAYER_SIZES,
        f_o_units=config_set2.GNN_BASELINE_F_O_LAYER_SIZES,
        phi_C_units=config_set2.SIAMESE_PHI_C_LAYER_SIZES,
        proj_units=config_set2.SIAMESE_PROJ_HEAD_LAYER_SIZES,
        temp=config_set2.SIMCLR_LOSS_TEMP,
        learning_rate=config_set2.SIAMESE_LEARNING_RATE,
    )
    ...
    simclr_model.fit(
        ...
        epochs=config_set2.SIAMESE_EPOCHS,
        steps_per_epoch=train_batches,
        validation_steps=val_batches,
        callbacks=config_set2.STANDARD_CALLBACKS(
            model_dir, 
        ),
    )
    ...
    # Train embeddings
    train_transformed = create_transformed_dataset(X_train, y_train, config_set2.BATCHSIZE, config_set2.N_FEATURES)
    ...
    # Val Embeddings
    val_transformed = create_transformed_dataset(X_val, y_val, config_set2.BATCHSIZE, config_set2.N_FEATURES)
    ...
    # Test Embeddings
    test_transformed = create_transformed_dataset(X_test, y_test, config_set2.BATCHSIZE, config_set2.N_FEATURES)
