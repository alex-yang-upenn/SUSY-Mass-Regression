...
def build_model_helper(config):
    ...
    optimizer = tf.keras.optimizers.Adam(learning_rate=config['GNN_TRANSFORMED_LEARNING_RATE'])
    model.compile(
        optimizer=optimizer,
        loss='mse',
        metrics=['mae', 'mape']
...
def main():
    ...
    model_dir = os.path.join(config["ROOT_DIR"], "gnn_transformed", f"model_{config['RUN_ID']}{config["DATASET_NAME"]}")
    ...
    train_transformed = create_transformed_dataset(
        X_train_scaled, y_train_scaled, batchsize=config['BATCHSIZE'], n_features=config['N_FEATURES']
    )
    val_batches = int(len(y_val_scaled) / config['BATCHSIZE'])
    val_transformed = create_transformed_dataset(
        X_val_scaled, y_val_scaled, batchsize=config['BATCHSIZE'], n_features=config['N_FEATURES']
    )
    test_batches = int(len(y_test_scaled) // config['BATCHSIZE'])
    test_transformed = create_transformed_dataset(
        X_test_scaled, y_test_scaled, batchsize=config['BATCHSIZE'], n_features=config['N_FEATURES']
    )
    ...
    model.fit(
        ...
        callbacks=get_no_stop_callbacks(model_dir, config['GNN_TRANSFORMED_LEARNING_RATE_DECAY']),
    )
