...
def main():    
    for name in os.listdir(config_set2.RAW_DATA_DIRECTORY):
        ...
        X_train, X_temp, y_mass_train, y_mass_temp, y_eta_train, y_eta_temp = train_test_split(
            X, y_mass, y_eta, test_size=config_set2.TRAIN_TEST_SPLIT, random_state=42
        )
        ...
        np.savez_compressed(os.path.join(config_set2.PROCESSED_DATA_DIRECTORY, "train", np_filename), X=X_train, y=y_mass_train, y_eta=y_eta_train)
        np.savez_compressed(os.path.join(config_set2.PROCESSED_DATA_DIRECTORY, "val", np_filename ), X=X_val, y=y_mass_val, y_eta=y_eta_val)
        np.savez_compressed(os.path.join(config_set2.PROCESSED_DATA_DIRECTORY, "test", np_filename), X=X_test, y=y_mass_test, y_eta=y_eta_test)
