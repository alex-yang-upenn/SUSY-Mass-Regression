...
def main():
    ...
    X_train, y_train, X_val, y_val, X_test, y_test = load_data_original(
    ...
    for i, s in enumerate(scalers):
        with open(
            os.path.join(config["PROCESSED_DATA_DIRECTORY"], f"x_scaler_{i}.pkl"), "wb"
        ) as f:
            pickle.dump(s, f)

    with open(
        os.path.join(config["PROCESSED_DATA_DIRECTORY"], "y_scaler.pkl"), "wb"
    ) as f:
