...
class SimCLRNTXentLoss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        sim_exp = tf.where(self_compare_mask, sim_exp, 0.0)  # Zero out self-comparisons
...
def vicreg_loss(
    ...
    cov_1 = tf.matmul(embed_1_centered, embed_1_centered, transpose_a=True) / (
        tf.cast(batch_size, tf.float32) - 1
    )
    ...
    cov_2 = tf.matmul(embed_2_centered, embed_2_centered, transpose_a=True) / (
        tf.cast(batch_size, tf.float32) - 1
    )
